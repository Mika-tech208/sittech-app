...
// Funcionários V1 — busca a faixa de datas que cobre a janela operacional
// completa (atual + anterior, reaproveitando calcularJanelaOperacional de
// Desvios V1) via obter_indicadores_producao/obter_paradas_producao —
// mesmas duas RPCs já usadas em Indicadores V1/Paradas V1/Desvios V1,
// mesmo shape de linha. O motor de Funcionários fatia tudo client-side.
...
export function useAnaliseFuncionariosProducao() {
  ...
  const buscar = useCallback(async (hoje: Date = new Date()) => {
    ...
    const params = {
      p_data_inicial: janelaOperacional.referencia.dataInicial,
    ...
    if (apontamentosResp.error || paradasResp.error) {
      setErro("Não foi possível carregar os dados de funcionários.");
      setApontamentos([]);
      setParadas([]);
      setLoading(false);
      return;
...
export type AnaliseFuncionariosProducaoHook = ReturnType<typeof useAnaliseFuncionariosProducao>;
