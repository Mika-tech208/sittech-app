...
// Desvios V1 — busca UMA única faixa de datas (a mais ampla das duas
// janelas de comparação, a estrutural: referência + atual = ~56 dias)
// via obter_indicadores_producao/obter_paradas_producao — as mesmas duas
// RPCs de Indicadores V1/Paradas V1, mesmo shape de linha. O motor de
// Desvios (src/features/producao-real/desvios/index.ts) fatia tudo
// client-side em janela operacional/estrutural, sem chamada nova ao
// banco por janela.
...
import { calcularJanelaEstrutural } from "@/features/producao-real/desvios/janelas";
...
export function useDesviosProducao() {
  ...
  const buscar = useCallback(async (hoje: Date = new Date()) => {
    setLoading(true);
    setErro(null);

    const janelaEstrutural = calcularJanelaEstrutural(hoje, JANELA_ESTRUTURAL_DIAS);
    const params = {
      p_data_inicial: janelaEstrutural.referencia.dataInicial,
      p_data_final: toISODate(hoje),
    ...
    if (apontamentosResp.error || paradasResp.error) {
      setErro("Não foi possível carregar os dados de desvios.");
      setApontamentos([]);
      setParadas([]);
      setLoading(false);
      return;
...
export type DesviosProducaoHook = ReturnType<typeof useDesviosProducao>;
