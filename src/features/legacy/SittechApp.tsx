...
import {
  Home, CalendarClock, Gauge, Package, Cog, Receipt, LineChart as LineChartIcon, Wallet, Clock, Users, Upload, Percent,
  DollarSign, TrendingUp, TrendingDown, Scale, Target, Sparkles, Bell, Crown, ClipboardList, Layers,
  CheckCircle2, AlertTriangle, ChevronDown, ChevronRight, Factory, Activity, PauseCircle, ClipboardCheck, Database, UserCog,
} from "lucide-react";
import {
...
import {
  STORAGE_KEY, CATEGORIAS, OPERACOES, PERIODOS_PADRAO, MESES, MESES_ABREV, PIE_COLORS, TITULOS_ABA, THEMES, USUARIOS_SEED,
} from "@/lib/constants";
...
export default function SittechApp() {
  ...
  const emptyPrevItemForm = { produtoId: "", quantidade: "", maquinasPorEtapa: {} };
  const [showPrevItemForm, setShowPrevItemForm] = useState(false);
  const [editingPrevItemId, setEditingPrevItemId] = useState(null);
  const [prevItemForm, setPrevItemForm] = useState(emptyPrevItemForm);
  const [showRealItemForm, setShowRealItemForm] = useState(false);
  const [editingRealItemId, setEditingRealItemId] = useState(null);
  const [realItemForm, setRealItemForm] = useState(emptyPrevItemForm);
  ...
  const totalFixo = useMemo(() => activeFixed.reduce((s, f) => s + Number(f.valor || 0), 0), [activeFixed]);
  ...
  // ---- produtos ----
  const duracaoPeriodoHoras = duracaoMediaPeriodo;

  const custoHoraPorOperacao = useMemo(() => {
    const map = {};
    porOperacao.forEach(([op, lista]) => {
      ...
      map[op] = total / ativosGrupo.length;
    });
    return map;
  }, [porOperacao, rateioPorHora]);

  function calcularIndicadoresProducao(produto) {
  ...
  function calcularCustoProducao(produto) {
    return calcularIndicadoresProducao(produto).custo;
  }
  function calcularMargem(produto) {
    const { custo, tempoTotalHoras } = calcularIndicadoresProducao(produto);
  ...
  function selecionarProdutoPrevItem(produtoId) {
    const produto = produtos.find((p) => p.id === produtoId);
    const sugestao = {};
    (produto?.roteiro || []).forEach((e) => {
      sugestao[e.id] = e.maquinasIds || [];
    });
    setPrevItemForm({ ...prevItemForm, produtoId, maquinasPorEtapa: sugestao });
  }
  function toggleMaquinaPrevItem(etapaId, maquinaId) {
    ...
    const linhasItens = semanaAtualRec.itens.map((it) => {
      ...
      return `
        <div class="item">
          ...
          ${etapaGargalo ? `<p class="callout">${etapaGargalo.nome} é a etapa mais lenta — tempo total estimado do produto: ${etapaGargalo.periodos.manha} período${etapaGargalo.periodos.manha !== 1 ? "s" : ""} de manhã e ${etapaGargalo.periodos.tarde} de tarde, ${textoDiasPeriodos(etapaGargalo.periodos.manha, etapaGargalo.periodos.tarde)}.</p>` : ""}
...
<body>
  ...
  <div class="resumo-final ${cabe ? "ok" : "alerta"}">
    ...
    a.href = url;
    a.download = `programacao-${semanaAtual}.html`;
  ...
  function resetRealItemForm() {
    ...
    setEditingRealItemId(null);
    setShowRealItemForm(false);
  }
  function submitRealItem() {
    if (!realItemForm.produtoId || !realItemForm.quantidade) return;
    const produto = produtos.find((p) => p.id === realItemForm.produtoId);
    if (!produto) return;
    const quantidadeNum = toNumber(realItemForm.quantidade);
    const item = { id: editingRealItemId || uid(), produtoId: produto.id, produtoNome: produto.nome, valorUnitario: produto.valorUnitario, quantidade: quantidadeNum };
    const itensAtuais = semanaAtualRec.itensRealizados || [];
    const novosItens = editingRealItemId
      ? itensAtuais.map((it) => (it.id === editingRealItemId ? item : it))
      : [...itensAtuais, item];
    upsertSemana({ itensRealizados: novosItens });
    resetRealItemForm();
  }
  function editRealItem(it) {
    setRealItemForm({ produtoId: it.produtoId, quantidade: String(it.quantidade) });
    setEditingRealItemId(it.id);
    setShowRealItemForm(true);
  }
  function deleteRealItem(id) {
    upsertSemana({ itensRealizados: (semanaAtualRec.itensRealizados || []).filter((it) => it.id !== id) });
  }
  function toggleMaquinaIndisponivelSemana(maquinaId) {
  ...
  function entrarModoSimulacao() {
    setItensSimulados(semanaAtualRec.itens.map((it) => ({ ...it })));
    setModoSimulacao(true);
  }
  function sairModoSimulacaoSemAplicar() {
    setModoSimulacao(false);
    setItensSimulados(null);
  }
  function aplicarSimulacao() {
    upsertSemana({ itens: itensSimulados });
    setModoSimulacao(false);
    setItensSimulados(null);
  }
  function alterarQuantidadeSimulada(itemId, novoValor) {
    setItensSimulados((prev) => prev.map((it) => (it.id === itemId ? { ...it, quantidade: toNumber(novoValor) } : it)));
  }
  ...
  function aplicarAjusteCapacidade() {
    ...
    if (modoSimulacao) {
      setItensSimulados(itensAjustados);
    } else {
      upsertSemana({ itens: itensAjustados });
    }
  ...
  const margemDesejadaNum = toNumber(margemDesejada) / 100;
  const divisorMeta = 1 - 0.09 - margemDesejadaNum;
  const metaInvalida = divisorMeta <= 0;
  const faturamentoMensalNecessario = !metaInvalida ? custoTotalMensalAtual / divisorMeta : 0;
  const faturamentoSemanalNecessario = faturamentoMensalNecessario / (52 / 12);
  const impostoMeta = faturamentoMensalNecessario * 0.09;
  const lucroMeta = faturamentoMensalNecessario - impostoMeta - custoTotalMensalAtual;
    ...
    <div className="stx-root">
      ...
      {loading ? (
        ...
        <div className="stx-login-screen">
          ...
          <div className="stx-login-card">
            ...
            <button type="button" className="stx-btn-primary" style={{ marginTop: 14, width: "100%" }} onClick={handleLogin} disabled={loginCarregando}>
      ...
      <div className="stx-layout">
        <div className="stx-sidebar">
          ...
          <button className={`stx-tab-v ${abaAtiva === "inicio" ? "active" : ""}`} onClick={() => setAbaAtiva("inicio")}><Home size={16} />Início</button>
          ...
          {gruposAbertos.gestao && (
            <>
              <button className={`stx-tab-v ${abaAtiva === "custos" ? "active" : ""}`} onClick={() => setAbaAtiva("custos")}><Wallet size={16} />Custos mensais</button>
              <button className={`stx-tab-v ${abaAtiva === "funcionarios" ? "active" : ""}`} onClick={() => setAbaAtiva("funcionarios")}><Users size={16} />Funcionários</button>
              <button className={`stx-tab-v ${abaAtiva === "produtos" ? "active" : ""}`} onClick={() => setAbaAtiva("produtos")}><Package size={16} />Produtos</button>
              <button className={`stx-tab-v ${abaAtiva === "maquinas" ? "active" : ""}`} onClick={() => setAbaAtiva("maquinas")}><Cog size={16} />Máquinas</button>
              <button className={`stx-tab-v ${abaAtiva === "horaEmpresa" ? "active" : ""}`} onClick={() => setAbaAtiva("horaEmpresa")}><Clock size={16} />Custo por hora</button>
            </>
          )}
          ...
          {gruposAbertos.financeiro && (
            <>
              <button className={`stx-tab-v ${abaAtiva === "faturamento" ? "active" : ""}`} onClick={() => setAbaAtiva("faturamento")}><Receipt size={16} />Faturamento mensal</button>
              <button className={`stx-tab-v ${abaAtiva === "bi" ? "active" : ""}`} onClick={() => setAbaAtiva("bi")}><LineChartIcon size={16} />Análise de faturamento</button>
            </>
          )}
          ...
          {gruposAbertos.planejamento && (
            <>
              <button className={`stx-tab-v ${abaAtiva === "previsao" ? "active" : ""}`} onClick={() => setAbaAtiva("previsao")}><CalendarClock size={16} />Previsão semanal</button>
              <button className={`stx-tab-v ${abaAtiva === "capacidade" ? "active" : ""}`} onClick={() => setAbaAtiva("capacidade")}><Gauge size={16} />Capacidade semanal</button>
            </>
          )}
          ...
          {gruposAbertos.producaoReal && (
            <>
              <button className={`stx-tab-v ${abaAtiva === "prVisaoGeral" ? "active" : ""}`} onClick={() => setAbaAtiva("prVisaoGeral")}><Factory size={16} />Visão Geral</button>
              <button className={`stx-tab-v ${abaAtiva === "prProdutividade" ? "active" : ""}`} onClick={() => setAbaAtiva("prProdutividade")}><Activity size={16} />Produtividade</button>
              <button className={`stx-tab-v ${abaAtiva === "prFuncionarios" ? "active" : ""}`} onClick={() => setAbaAtiva("prFuncionarios")}><Users size={16} />Funcionários</button>
              <button className={`stx-tab-v ${abaAtiva === "prDesvios" ? "active" : ""}`} onClick={() => setAbaAtiva("prDesvios")}><AlertTriangle size={16} />Desvios</button>
              <button className={`stx-tab-v ${abaAtiva === "prParadas" ? "active" : ""}`} onClick={() => setAbaAtiva("prParadas")}><PauseCircle size={16} />Paradas</button>
              <button className={`stx-tab-v ${abaAtiva === "prValidacao" ? "active" : ""}`} onClick={() => setAbaAtiva("prValidacao")}><ClipboardCheck size={16} />Validação da Previsão</button>
              <button className={`stx-tab-v ${abaAtiva === "prDadosImportados" ? "active" : ""}`} onClick={() => setAbaAtiva("prDadosImportados")}><Database size={16} />Dados Importados</button>
            </>
          )}
          ...
          {gruposAbertos.administracao && (
            <>
              {usuarioLogado?.papel === "admin" && (
                <button className={`stx-tab-v ${abaAtiva === "usuarios" ? "active" : ""}`} onClick={() => setAbaAtiva("usuarios")}><UserCog size={16} />Usuários</button>
              )}
              <button className={`stx-tab-v ${abaAtiva === "importar" ? "active" : ""}`} onClick={() => setAbaAtiva("importar")}><Upload size={16} />Importar dados</button>
            </>
          )}

          <div className="stx-sidebar-meta-card" onClick={() => setAbaAtiva("inicio")}>
      ...
      <div className="stx-header">
        ...
        <div className="stx-header-right">
          <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
            ...
            <button className="stx-theme-toggle" onClick={() => setTema((t) => (t === "dark" ? "light" : "dark"))}>
              {tema === "dark" ? "☀ Modo claro" : "🌙 Modo escuro"}
            </button>
            <button className="stx-theme-toggle" onClick={() => { setMinhaContaAberta(true); setMinhaContaMsg(""); }}>👤 Minha conta</button>
            <button className="stx-theme-toggle" onClick={() => setAutenticado(false)}>Sair</button>
      ...
      {abaAtiva === "previsao" && (
          ...
          <div className="stx-month-nav" style={{ marginBottom: 18, justifyContent: "space-between" }}>
            ...
            {semanaAtualRec.itens.length > 0 && (
              <div style={{ textAlign: "right", display: "flex", gap: 8, alignItems: "flex-start" }}>
                {!modoSimulacao ? (
                  <button className="stx-btn-secondary" onClick={entrarModoSimulacao}>🧪 Modo simulação</button>
                ) : (
                  <>
                    <button className="stx-btn-secondary" onClick={sairModoSimulacaoSemAplicar}>Sair sem aplicar</button>
                    <button className="stx-btn-primary" onClick={aplicarSimulacao}>Aplicar simulação</button>
                  </>
                )}
                <div>
                  <button className="stx-btn-secondary" onClick={gerarPDFSemana}>GERAR PDF</button>
                  ...
                  {itensSimulados.map((it) => (
                    <div className="stx-simulacao-item" key={it.id}>
                      ...
                      <input
                        ...
                        type="number"
                        value={it.quantidade}
                        onChange={(e) => alterarQuantidadeSimulada(it.id, e.target.value)}
                      />
            ...
            <div>
              <div className="stx-panel">
                ...
                {produtos.filter((p) => p.ativo).length === 0 ? (
                  ...
                  <>
                    {!showPrevItemForm && (
                      <button className="stx-add-btn blueprint" onClick={() => setShowPrevItemForm(true)}>+ Novo item</button>
                    )}
                    {showPrevItemForm && (
                      <div className="stx-form">
                        <div className="stx-form-full">
                          ...
                          <select
                            className="stx-select"
                            value={prevItemForm.produtoId}
                            onChange={(e) => selecionarProdutoPrevItem(e.target.value)}
                          >
                        ...
                        <div>
                          ...
                          <input
                            className="stx-input"
                            value={prevItemForm.quantidade}
                            onChange={(e) => setPrevItemForm({ ...prevItemForm, quantidade: e.target.value })}
                            onKeyDown={(e) => e.key === "Enter" && submitPrevItem()}
                            placeholder="Ex: 10"
                            inputMode="decimal"
                            autoFocus
                          />
                        ...
                        {prevItemForm.produtoId && (() => {
                          const produtoSel = produtos.find((p) => p.id === prevItemForm.produtoId);
                          const roteiro = produtoSel?.roteiro || [];
                          if (roteiro.length === 0) {
                            return <p className="stx-panel-sub stx-form-full" style={{ margin: 0 }}>Esse produto não tem fluxo de produção cadastrado — não dá pra escolher máquinas nem calcular viabilidade.</p>;
                          }
                          return (
                            <div className="stx-custos-builder">
                              ...
                              {roteiro.map((etapa) => {
                                const maquinasDaOperacao = maquinas.filter(
                                  (m) => m.operacao === etapa.operacao && m.ativo && !(semanaAtualRec.maquinasIndisponiveis || []).includes(m.id)
                                );
                                const selecionadas = prevItemForm.maquinasPorEtapa[etapa.id] || [];
                                return (
                                        ...
                                        {maquinasDaOperacao.map((m) => (
                                          <label className="stx-maquina-chip" key={m.id}>
                                            <input
                                              ...
                                              onChange={() => toggleMaquinaPrevItem(etapa.id, m.id)}
                                            />
                                            {m.nome}
                                          </label>
                                        ))}
                        ...
                        {prevItemForm.produtoId && (() => {
                          const produtoSel = produtos.find((p) => p.id === prevItemForm.produtoId);
                          if (!produtoSel || !(produtoSel.roteiro || []).length) return null;
                          const viabAoVivo = calcularViabilidadeItem({
                          ...
                          return (
                            <div className="stx-form-full stx-resumo-ao-vivo" style={{ borderColor: viabAoVivo.atingivel ? "var(--accent)" : "var(--warning)" }}>
                              ...
                              {!viabAoVivo.atingivel && toNumber(prevItemForm.quantidade) > 0 && (
                                <> Isso é menos que a quantidade pedida ({toNumber(prevItemForm.quantidade)}) — falta capacidade em <b>{viabAoVivo.gargalo}</b>.</>
                              )}
                            </div>
                          );
                        })()}
                        ...
                        <div className="stx-form-actions">
                          <button type="button" className="stx-btn-primary" onClick={submitPrevItem}>
                            {editingPrevItemId ? "Salvar alterações" : "Adicionar item"}
                          </button>
                          <button type="button" className="stx-btn-secondary" onClick={resetPrevItemForm}>Cancelar</button>
                        </div>
                      </div>
                    )}
                  </>
                    ...
                    return (
                      <div className="stx-entry" key={it.id}>
                        ...
                        <div className="stx-entry-right">
                          ...
                          <button className="stx-icon-btn" title="Editar" onClick={() => editPrevItem(it)}>✎</button>
                          <button className="stx-icon-btn danger" title="Excluir" onClick={() => deletePrevItem(it.id)}>✕</button>
                        </div>
                      </div>
                    );
                  })
                ...
                {semanaAtualRec.itens.length > 0 && (
                  <>
                    <p className="stx-custos-total" style={{ marginTop: 4 }}>
                      Funcionários necessários pra essa programação: <b>{calcularFuncionariosTotalSemana(semanaAtualRec.itens)}</b> de {funcionariosAtivos.length} ativos
                    </p>
                    {(() => {
                      const horasPessoaDisponiveis = funcionariosAtivos.length * horasPorMaquinaSemana;
                      const horasPessoaDemandadas = analiseCapacidadeSemana.maquinas.reduce((s, m) => s + m.horasNecessarias, 0);
                      ...
                      return (
                        <p className="stx-custos-total" style={{ marginTop: 4 }}>
                          ...
                          {" "}<span style={{ fontWeight: 400, color: "var(--text-muted)", fontSize: 12 }}>
                            ({horasPessoaDemandadas.toFixed(1)}h demandadas de {horasPessoaDisponiveis.toFixed(1)}h disponíveis nos {funcionariosAtivos.length} funcionários)
                          </span>
                        </p>
                      );
                    })()}
              ...
              <div className="stx-panel">
                ...
                {produtos.filter((p) => p.ativo).length === 0 ? (
                  ...
                  <>
                    {!showRealItemForm && (
                      <button className="stx-add-btn" onClick={() => setShowRealItemForm(true)}>+ Novo item</button>
                    )}
                    {showRealItemForm && (
                      <div className="stx-form">
                        <div className="stx-form-full">
                          ...
                          <select
                            className="stx-select"
                            value={realItemForm.produtoId}
                            onChange={(e) => setRealItemForm({ ...realItemForm, produtoId: e.target.value })}
                          >
                            <option value="">Selecione um produto…</option>
                            {produtos.filter((p) => p.ativo).map((p) => (
                              <option key={p.id} value={p.id}>{p.nome} ({formatBRL(p.valorUnitario)})</option>
                        ...
                        <div>
                          ...
                          <input
                            className="stx-input"
                            value={realItemForm.quantidade}
                            onChange={(e) => setRealItemForm({ ...realItemForm, quantidade: e.target.value })}
                            onKeyDown={(e) => e.key === "Enter" && submitRealItem()}
                            placeholder="Ex: 10"
                            inputMode="decimal"
                            autoFocus
                          />
                        ...
                        <div className="stx-form-actions">
                          <button type="button" className="stx-btn-primary" onClick={submitRealItem}>
                            {editingRealItemId ? "Salvar alterações" : "Adicionar item"}
                          </button>
                          <button type="button" className="stx-btn-secondary" onClick={resetRealItemForm}>Cancelar</button>
                        </div>
                      </div>
                    )}
                  </>
                ...
                {loading ? (
                  <div className="stx-empty">Carregando…</div>
                ) : (semanaAtualRec.itensRealizados || []).length === 0 ? (
                  <div className="stx-empty">Nenhum item realizado lançado nessa semana ainda.</div>
                ) : (
                  semanaAtualRec.itensRealizados.map((it) => (
                    <div className="stx-entry" key={it.id}>
                      <div>
                      ...
                      <div className="stx-entry-right">
                        <span className="stx-entry-value">{formatBRL(it.quantidade * it.valorUnitario)}</span>
                        <button className="stx-icon-btn" title="Editar" onClick={() => editRealItem(it)}>✎</button>
                        <button className="stx-icon-btn danger" title="Excluir" onClick={() => deleteRealItem(it.id)}>✕</button>
                      </div>
                    </div>
                  ))
                )}
            ...
            <div>
              <div className="stx-panel">
                ...
                <div className="stx-rateio-line">
                  ...
                  <span className="v">{formatBRL(valorPrevistoSemana)}</span>
                </div>
                <div className="stx-rateio-line">
                  <span className="l">Realizado</span>
                  <span className="v">{formatBRL(valorRealizadoSemana)}</span>
                </div>
                <div className="stx-rateio-line">
                  <span className="l">Diferença</span>
                  <span className="v" style={diferencaSemana < 0 ? { color: "var(--danger)" } : { color: "var(--blueprint)" }}>
                    {diferencaSemana >= 0 ? "+" : ""}{formatBRL(diferencaSemana)}
                  </span>
                </div>
                <div className="stx-destaque-box" style={{ marginTop: 12 }}>
                  <p className="stx-destaque-label">Concluído da previsão</p>
                  <p className="stx-destaque-value">{percentualConcluidoSemana.toFixed(1)}%</p>
                  <p className="stx-destaque-sub">{formatBRL(valorRealizadoSemana)} de {formatBRL(valorPrevistoSemana)} previstos</p>
                </div>
              </div>
        ...
        {showAjustarModal && (
          <div className="stx-modal-backdrop" onClick={() => setShowAjustarModal(false)}>
            <div className="stx-modal-card" onClick={(e) => e.stopPropagation()}>
              ...
              <div className="stx-form-actions" style={{ marginTop: 16, justifyContent: "flex-end" }}>
                <button type="button" className="stx-btn-secondary" onClick={() => setShowAjustarModal(false)}>Cancelar</button>
                <button type="button" className="stx-btn-primary" onClick={aplicarAjusteCapacidade}>Aplicar ajuste</button>
              </div>
            </div>
          </div>
        )}
                    ...
                    <div>
                      ...
                      <p className="stx-entry-meta">
                        {r.semProduto
                          ...
                          : `previsto ${r.quantidade} · aloca ${Math.floor(r.quantidadeAlocada)}${r.produto ? ` · ${calcularMargem(r.produto).lucroHora.toFixed(2)} R$/h` : ""}`}
                      </p>
                    </div>
                    <div className="stx-entry-right">
      ...
      {minhaContaAberta && usuarioLogado && (
        <div className="stx-modal-backdrop" onClick={() => setMinhaContaAberta(false)}>
          <div className="stx-modal-card" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="stx-form-actions" style={{ justifyContent: "flex-end" }}>
              <button type="button" className="stx-btn-secondary" onClick={() => setMinhaContaAberta(false)}>Fechar</button>
              <button type="button" className="stx-btn-primary" onClick={alterarMinhaSenha}>Salvar nova senha</button>
            </div>
          </div>
        </div>
      )}
